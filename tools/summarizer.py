...
chat_35_0 = ChatOpenAI(openai_api_key=ai_key, temperature=0, verbose=True)

chat_4_0 = ChatOpenAI(model="gpt-4", openai_api_key=ai_key, temperature=0, verbose=True)
...
async def get_all_summaries(paper: str):
    ...
    summary_funcs = [
        ...
        get_results_summary,
        get_conclusion_summary,
        get_future_work_summary,
...
async def main(paper):
    ...
    sections_and_summaries["main_summary"] = await get_summary_from_all_summaries(
        sections_and_summaries.values()
    )
