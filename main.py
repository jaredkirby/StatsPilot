...
def main():
    ...
    # Set up tabs
    tab1, tab2, tab3, tab4 = st.tabs(["Explain a Concept or Term", "Answer an Exam Question", "Ask a Question", "Expression to English"])
...
def handle_exam_question_tab(samples):
    ...
    user_input = st.text_area("Input Exam Question", sample, height=275, key="exam_question_input")
    temperature = 0
    model = 'gpt-4'
...
def handle_question_tab(samples, skill_levels):
    ...
    skill_level = st.select_slider("Select your skill level:", list(skill_levels.keys()), key="question_skill_level")
    temperature = 1
    model = 'gpt-4'
    ...
    if question_button:
        ...
        user_question.get_user_question_answer(question_chat, user_input, skill_levels[skill_level])
...
def handle_explain_tab(samples, skill_levels):
    ...
    skill_level = st.select_slider("Select level of explanation:", list(skill_levels.keys()), key="skill_level")
    temperature = 1
    model = 'gpt-4'
...
def handle_expression_tab(samples):
    ...
    temperature = 0.75
    model = 'gpt-4'
    expression_button = st.button("Translate Expression to English", key="expression_button")
...
def handle_summarize_tab(samples):
    ...
    temperature = 1
    model = 'gpt-4'
    user_input = st.text_area("Input Research Paper", sample, height=275, key="paper_input")
